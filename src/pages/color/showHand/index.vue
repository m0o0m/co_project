<template>
  <div class="tempDiv tempDivMargin">
    <Headered
    ...
    <DrawPoker  v-if="isOpenLottery"
                :open-lottery-result="openLotteryResult"
                :creditId="creditId"></DrawPoker>
  </div>
</template>

<script type="text/babel">
  import $ from 'jquery'
  import '../../../assets/scss/common/lottery/showHand/index.scss';
  ...
  import PaiGow from './paiGow.vue'
  import DrawPoker from '../../../components/drawAlotteryPoker.vue'
  ...
  export default {
    data () {
      return {
        ...
        classAdata: '',//一级数据
        creditId: 1,//彩种ID
        icon: require('../../../assets/images/index/icon/15.png'),//彩种图标
        rule: '',//彩种规则
    ...
    mounted () {
	  ...
	  let that = this;
	  that.$on('openLottery', (data) => {
		that.isOpenLottery = true;
		that.openLotteryResult = data;
	  });

      that._Util.visibilityChange(that, that.gameId);
    },
    methods : {
...
<style lang="scss">
  .pcdd .paiGow.sHGow {
    background: linear-gradient(top,#33626c,#2c5c64); background: -webkit-linear-gradient(top,#33626c,#2c5c64);
  }
</style>
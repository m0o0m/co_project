<template>
    <div class="ColorLotteryMuned ColorLotteryMuned12">
        <div class="ColorLottery_mianTop ColorLottery_mianTop12" :class="{'ColorLottery_mianTop12_0':lotteryOffsColorID == '275'}">
            <img :src="ColorImgTop"/>
        </div>
        <p class="lottery12" :class="{'lottery12_0':lotteryOffsColorID == '275'}"><img :src="ColorImgRight"/></p>
        <div class="XGLottery_list" v-if="lotteryOffsColorID == '275'">
            <ul>
                <li v-for="(lotData,nav_index) in deafNum" :class="{'on':nav_index == navIndex}"
                    @click="Navigation(lotData, nav_index)"><p>{{lotData.name}}</p></li>
            </ul>
        </div>
        <div class="ColorLottery_mian ColorLottery_mian12" :class="{'ColorLot_mian12':lotteryOffsColorID == '275'}">
            <div class="ColorLottery_mcontent ColorLottery_mcontent12" v-for="(lotData,index) in deafNum" v-if="lotteryOffsColorID == '299' && index == 0 || lotteryOffsColorID != '299'"
                 :class="{'colorClsLottery': lotteryOffsColorID == '275' , 'colorClsOneLottery': navIndex == index}">
                <!--<div class="ColorLottery_mcTitle ColorLottery_mcTitle12"><p>{{lotData.name}}</p></div>-->
                <div class="HalfwaveCls" v-if="lotteryOffsColorID == '293'">
                    <ul>
                        <li v-for="(Half,half_index) in Halfwave" :class="{'on':half_index == HalfIndex}"
                            @click="HalfwaveFun(half_index)"><p>{{Half}}</p></li>
                    </ul>
                </div>
                <div class="ColorLottery_mslist12 ColorLotterys">
                    <ul :class="{'colorzodiac':lotteryOffsColorID == '289' || lotteryOffsColorID == '291' || lotteryOffsColorID == '293' || lotteryOffsColorID == '295','colorzodiac04': lotteryOffsColorID == '293'}"
                        v-if="lotteryOffsColorID != '299'">

                        <li v-for="(lData,lData_index) in (items.length ? items : lotData.played)"
                            :class="{'liLotERY':lotteryOffsColorID == '293', 'liLotERY00':lData_index < 4 && HalfIndex == 0,'liLotERY01':lData_index >= 4 && lData_index < 8 && HalfIndex == 1, 'liLotERY02':lData_index >= 8 && HalfIndex == 2}">
                            <!--<li v-for="(lData,lData_index) in items">-->
                            <article
                                    @click="playchecked($event, lData, items.length ? groupId : lotData.id, lData.name, index, lotData, 'ColorLot_article', lData_index)"
                                    class="ColorLot_article">
                                <section :class="{'ColorLot_Colorsection': lotteryOffsColorID == '293'}">
                                    <p><span v-if="lotteryOffsColorID != '289' && lotteryOffsColorID != '291'">{{lData.name}}</span><span
                                            v-if="lotteryOffsColorID == '289' || lotteryOffsColorID == '291'" class="colorZodatr"><img
                                            :src="lData.zodiacImg"/></span> / {{computeOdds(lData)}}</p>
                                    ...
                                    <!--半波-->
                                    <div  class="zodiaoCls05" v-if="lotteryOffsColorID == '293'">
                                        <span v-for="data in lData.zodiacArr"
                                              :class="{'red':lData_index < 4,'green':lData_index >= 4 && lData_index <= 7, 'blue': lData_index >= 8 && lData_index <= 11}">{{data}}</span>
                                    </div>
                                ...
                                <div class="zodiaoCls" v-if="lotteryOffsColorID == '289' || lotteryOffsColorID == '291'">
                                    <!--<p class="playMarryImg imggs" v-if="lData.active && !$parent.stopBet"><img src=""/></p>-->
                                    <p v-for="data in lData.zodiacArr"
                                       :class="{'green':data=='22' || data=='21' || data=='33' || data=='32' || data=='44' || data=='43' || data=='06' || data=='05' || data=='17' || data=='16' || data=='28' || data=='27' || data=='39' || data=='38' || data=='49' || data=='11','blue': data=='10' || data=='09' || data=='20' || data=='31' || data=='42' || data=='41' || data=='04' || data=='03' || data=='15' || data=='14' || data=='26' || data=='25' || data=='37' || data=='36' || data=='48' || data=='47'}">
                                        {{data}}</p>

                                </div>
                                <!--尾数-->
                                <div class="zodiaoCls" v-if="lotteryOffsColorID == '295'">
                                    <!--<p class="playMarryImg imggs" v-if="lData.active && !$parent.stopBet"><img src=""/></p>-->
                                    <p v-for="data in lData.zodiacArr"
                                       :class="{'green':data=='11' || data=='21' || data=='22' || data=='32' || data=='33' || data=='43' || data=='44' || data=='05' || data=='06' || data=='16' || data=='17' || data=='27' || data=='28' || data=='38' || data=='39' || data=='49','blue': data=='10' || data=='20' || data=='31' || data=='41' || data=='42' || data=='03' || data=='04' || data=='14' || data=='15' || data=='25' || data=='26' || data=='36' || data=='37' || data=='47' || data=='48' || data=='09'}">
                                        {{data}}</p>
                                </div>
                    ...
                    <ul class="XGlI combat_lustrating" v-else>
                        <!--龙虎斗-->
        ...
        lotteryColorData: [
          {
            title: "第一球",
            data: [
              {name: "0", odds: "12.348", marry: ""}, {name: "1", odds: "12.348", marry: ""}, {
                name: "2",
                odds: "12.348",
                marry: ""
              }, {name: "3", odds: "12.348", marry: ""}, {name: "4", odds: "12.348", marry: ""}, {
                name: "5",
                odds: "12.348",
                marry: ""
              }, {name: "6", odds: "12.348", marry: ""}, {name: "7", odds: "12.348", marry: ""}, {
                name: "8",
                odds: "12.348",
                marry: ""
              }, {name: "9", odds: "12.348", marry: ""}, {name: "10", odds: "12.348", marry: ""}
            ]
          },
          {
            title: "第二球",
      ...
      deafNum() {
        if (typeof(this.classAdata) == "string" || typeof(this.classAdata.played) == 'undefined') {
          ...
          let zodiacDev = zodiacIndex.length,
            zodiacStr = "";
          if (this.creditId == '3' && (this.lotteryOffsColorID == '289' || this.lotteryOffsColorID == '291')) {
          ...
          if (this.creditId == '3' && (this.lotteryOffsColorID == '293' || this.lotteryOffsColorID == '295')) {
            for (var i = 0; i < zodiacDev; i++) {
              zodiacIndex[i].zodiacArr = zodiacIndex[i].num.split(',');
            }
          }
          if (this.creditId == '3' && this.lotteryOffsColorID == '299') {
            this.lotteryilustrating = this.classAdata.played[this.oneCredId].item;
          }

          if (this.lotteryOffsColorID == '275') {
            this.items = this.classAdata.played[this.oneCredId].item[this.navIndex].played;
          } else {
            this.items = [];
          }

          if (this.lotteryOffsColorID == '299') {
    ...
    methods: {
      playchecked: function (e, play, playedSubtypeId, playedSubtypeName, index, colorful, className, lData_index,lus_index) {
        ...
        //this.deliverOST(e, index, lData_index);
          setTimeout(function () {
            if (_this.lotteryOffsColorID == '299'){

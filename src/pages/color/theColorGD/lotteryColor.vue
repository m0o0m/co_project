    ...
    <div class="colorLotteryMarginOne">
      ...
      <div class="ColorLottery_mian ColorLottery_mian23">
        <div class="ColorLottery_mcontent pcdd_bjpk_mun" v-for="(lotData,index) in deafNum">
          <div class="ColorLottery_mcTitle ColorLottery_mcTitle23" v-if="lotteryOffsColorID != '179'"><p>
            {{lotData.name}}</p></div>
          <div class="ColorLottery_mslist ColorLottery_mslist23 pcdd_mun_list_bjpk10">
            <ul :class="{'ColorLotCs': lotteryOffsColorID == '190'}"
                v-if="lotteryOffsColorID != '179'">
              <li v-for="(lData,lData_index) in lotData.played">
                <article
                    @click="playchecked($event, lData, lotData.id, lData.name, index, lotData, 'ColorLot_article', lData_index)"
                  ...
                  <section>
                    ...
                    <p class="playMarryImg" v-if="lData.active && !$parent.stopBet"><img
                  ...
                  <section>
                    <article class="colorLot_articleP" v-for="(lus, lus_index) in lustrating.played"
                      ...
                      <p :class="{'amount': lus.amount > 0 || $parent.stopBet}">
                        {{_LotteryUtil.showAmount($parent.classAdata.status, $parent.stopBet, lus.amount) || 0}}</p>
                      <p class="playMarryImg dragon" v-if="lus.active && !$parent.stopBet"><img
                          src="../../../assets/images/small01.png" alt=""></p>
                    </article>
                  </section>
                </article>
	...
	export default {
		...
		watch: {
			'oneCredId'(num) {
				...
				this.lotteryOffsColorID = this.$parent.$refs.headerRef.lotteryTypeId;
				this.lotteryNavID = this.lotteryOffsColorID;
			},
			...
			// 'lotteryOffsColorID'() {
			// 	this.lotteryOffsColorID = this.lotteryNavID;
			// }
		}
	}
</script>

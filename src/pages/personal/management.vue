<template>
    <div class="managMementCls">
        ...
        <div v-if="$route.query.phone && ($route.query.type == 2 || $route.query.type == 3)" class="per_conter per_conterstop cons3" strle="display:none;">
            <div class="monum">
                <p>手机号</p>
                <p>
                    <input type="text" :value="_Util.formatPhone($route.query.phone || '')" readonly style="border: none; background: transparent; font-size: 0.85rem; color: #FFDC99; font-family: Arial; text-indent: 0;">
                </p>
            </div>
	...
	methods: {
	  save() {
        ...
        that._Util.post(that, that._Api.POST_COMMON_BIND_PHONE, {
		  ...
		  if (that.$route.query.nextRouteName) {
            that.$router.replace({name: that.$route.query.nextRouteName, query: {phone: that.phone, code: that.$refs.captchaRef.code}});
		  } else {
	  ...
	  cashNext() {
		...
		that._Util.post(that, that._Api.POST_COMMON_VALIDATE_CODE, {
          ...
          that.$router.replace({name: 'withdrawals', query: {phone: that.$route.query.phone, code: that.$refs.captchaRef.code, type: that.$route.query.type}});
		});
      },
      customerServer(){
	      let that = this;
